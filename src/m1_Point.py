...
class Point(object):
    ...
    def move_to(self,x,y):
        ...
        self.distt = self.distt + math.sqrt(
            (self.x-x) ** 2 +
            (self.y-y) ** 2
        )
        self.x = x
    ...
    def get_distance_from(self,pt):
        dis = math.sqrt((self.x-pt.x) ** 2 + (self.y-pt.y) ** 2)
        return dis

    def get_distance_from_start(self):
        disfs = math.sqrt((self.x - self.startx) ** 2 +
                          (self.y - self.starty) ** 2)
        return disfs
